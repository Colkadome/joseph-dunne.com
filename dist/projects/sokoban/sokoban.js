...
Sokoban.prototype.getNode = function (c) {
  switch (c) {
    ...
    case types.boxOverTrap:
      return display.boxOverTrap;

    case types.player:
      return display.player;

    case types.playerOverTrap:
      return display.playerOverTrap;
...
Sokoban.prototype.undoMove = function () {
  ...
  var xDir = move[0];
  var yDir = move[1];
  var pushed = move[2]; // Player position after move.

  var playerPos = this.findPlayerPosition();
  var pX = playerPos[0];
  var pY = playerPos[1];
  var p = this.getItemAt(pX, pY); // Item behind player.

  var rX = pX - xDir;
  var rY = pY - yDir;
  var r = this.getItemAt(rX, rY); // Move player backwards.
